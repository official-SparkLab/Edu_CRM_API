  ...
  try {
    if ([STATUS.INACTIVE, STATUS.DELETE].includes(req.user.status)) {
      return res
        ...
        .json({
          success: false,
          message: "Only Active User can create admissionCourse.",
        });
    }

    const admissionCourseData = { ...req.body };
    const User = require("../../users/user.model");
    if (!admissionCourseData.section_id) {
      return res
        .status(400)
    ...
    if ([STATUS.INACTIVE, STATUS.DELETE].includes(addedByUser.status)) {
      return res
        ...
        .json({
          success: false,
          message:
            "Inactive or deleted users cannot create an admissionCourse.",
        });
    }
    ...
    await cacheService.set(
      `${CACHE_PREFIX}${admissionCourse.adm_course_id}`,
      toPlain(admissionCourse)
    );
  ...
  try {
    if ([STATUS.INACTIVE, STATUS.DELETE].includes(req.user.status)) {
      return res
        ...
        .json({
          success: false,
          message: "Only Active User can update admissionCourse.",
        });
    }
    ...
    if ([STATUS.INACTIVE, STATUS.DELETE].includes(addedByUser.status)) {
      return res
        ...
        .json({
          success: false,
          message:
            "Inactive or deleted users cannot update an admissionCourse.",
        });
    }
    ...
    if (!admissionCourse) {
      return res
        .status(404)
        .json({ success: false, message: "AdmissionCourse not found" });
    }
  ...
  try {
    if ([STATUS.INACTIVE, STATUS.DELETE].includes(req.user.status)) {
      return res
        ...
        .json({
          success: false,
          message: "Only Active User can delete admissionCourse.",
        });
    }
    ...
    res.json({
      success: true,
      message: "AdmissionCourse soft deleted (status=0)",
    });
  } catch (err) {
    next(err);
  }
