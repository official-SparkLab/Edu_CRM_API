  ...
  try {
    if ([STATUS.INACTIVE, STATUS.DELETE].includes(req.user.status)) {
      return res
        ...
        .json({
          success: false,
          message: "Only Active User can create admissionService.",
        });
    }
    ...
    if (!admissionServiceData.section_id) {
      return res
        .status(400)
    ...
    if ([STATUS.INACTIVE, STATUS.DELETE].includes(addedByUser.status)) {
      return res
        ...
        .json({
          success: false,
          message:
            "Inactive or deleted users cannot create an admissionService.",
        });
    }
    ...
    const admissionService = await AdmissionService.create(
      admissionServiceData
    );

    // invalidate cache
    await cacheService.del(`${LIST_CACHE_PREFIX}${admissionService.branch_id}`);
    await cacheService.set(
      `${CACHE_PREFIX}${admissionService.adm_service_id}`,
      toPlain(admissionService)
    );
  ...
  try {
    if ([STATUS.INACTIVE, STATUS.DELETE].includes(req.user.status)) {
      return res
        ...
        .json({
          success: false,
          message: "Only Active User can update admissionService.",
        });
    }
    ...
    const branch = await Branch.findOne({
      where: {
        branch_id,
        status: { [Op.in]: [STATUS.ACTIVE, STATUS.INACTIVE] },
      },
    });
    if (!branch) {
      return res
    ...
    if ([STATUS.INACTIVE, STATUS.DELETE].includes(addedByUser.status)) {
      return res
        ...
        .json({
          success: false,
          message:
            "Inactive or deleted users cannot update an admissionService.",
        });
    }
    ...
    if (!admissionService) {
      return res
        .status(404)
        .json({ success: false, message: "AdmissionService not found" });
    }
  ...
  try {
    if ([STATUS.INACTIVE, STATUS.DELETE].includes(req.user.status)) {
      return res
        ...
        .json({
          success: false,
          message: "Only Active User can delete admissionService.",
        });
    }
    ...
    res.json({
      success: true,
      message: "AdmissionService soft deleted successfully",
    });
  } catch (err) {
    next(err);
  }
